...
class User extends CActiveRecord
{
	...
	public function rules()
	{
		...
		if(UserModule::isAdmin()){
			return ((Yii::app()->getModule('user')->isAdmin())?array(
				...
				array('username, email, createtime, lastvisit, superuser, manager, employe, status', 'required'),
				array('createtime, lastvisit, superuser, manager, employe, status', 'numerical', 'integerOnly'=>true),
		...
		//Manager
			...
			return ((Yii::app()->getModule('user')->isManager())?array(
				...
				array('status', 'in', 'range'=>array(self::STATUS_NOACTIVE,self::STATUS_ACTIVE,self::STATUS_BANED)),
				array('superuser', 'in', 'range'=>array(0,1)),
				array('username, email, createtime, lastvisit, superuser, manager, employe, status, idcreateur', 'required'),
				array('createtime, lastvisit, superuser, manager, employe, status, idcreateur', 'numerical', 'integerOnly'=>true),
			):((Yii::app()->user->id==$this->id)?array(
				array('username, email', 'required'),
				array('username', 'length', 'max'=>20, 'min' => 3,'message' => UserModule::t("Identifiant incorrect (Doit contenir entre 3 et 20 caractères).")),
				array('email', 'email'),
	...
	public function scopes()
    {
        return array(
            ...
            'notsafe'=>array(
            	'select' => 'id, username, password, email, activkey, createtime, lastvisit, superuser, manager, employe, status, idcreateur',
            ),
        );
    }
    ...
    {
        return array(
            'select' => ' id,  username, email, createtime, lastvisit, superuser, manager, employe, status, idcreateur',
        );
    }

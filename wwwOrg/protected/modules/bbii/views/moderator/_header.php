...
<div id="bbii-header">
	...
	<?php $messages = BbiiMessage::model()->inbox()->unread()->count('sendto = '.Yii::app()->user->id); ?>
		...
		<?php 
			...
			if($this->isAdmin()) echo ' | ' . CHtml::link(CHtml::image($this->module->getRegisteredImage('config.png'), Yii::t('BbiiModule.bbii', 'Forum settings'), array('title'=>Yii::t('BbiiModule.bbii', 'Forum settings'),'style'=>'vertical-align:bottom;')), array('setting/layout'));
		?>
		</div>
	<?php endif; ?>
